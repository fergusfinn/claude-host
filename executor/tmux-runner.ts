...
export class TmuxRunner {
  ...
  createJob(opts: CreateJobOpts): { name: string; command: string } {
    ...
    writeFileSync(launcherScript, [
      ...
      "  echo \"\"",
      "  if [ $i -eq 1 ]; then",
      "    claude -p --dangerously-skip-permissions --session-id \"$SID\" \"$PROMPT\" > \"$OUTPUT_FILE\" 2>&1",
      "  else",
      "    claude -p --dangerously-skip-permissions --resume \"$SID\" \"Continue working on the task. Output <promise>DONE</promise> when complete.\" > \"$OUTPUT_FILE\" 2>&1",
      "  fi",
      "  cat \"$OUTPUT_FILE\"",
